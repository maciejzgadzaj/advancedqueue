...
class AdvancedQueueExampleForm extends FormBase {
  ...
  public function submitCreateItems(array &$form, FormStateInterface $form_state) {
    ...
    foreach ($form_state->getValue('queue_name') as $queue_name) {
      ...
      drupal_set_message(\Drupal::translation()
        ->formatPlural($form_state->getValue('create_count'), 'Created 1 item @item_ids in queue %queue_name.', 'Created @count items @item_ids in queue %queue_name.', ['@item_ids' => implode(', ', $item_ids), '%queue_name' => $queue_name]));
    }
  }
