...
class AdvancedQueueItem extends ContentEntityBase implements AdvancedQueueItemInterface {
  ...
  public static function getStatusConsoleTag($status) {
    $tags = [
      self::STATUS_QUEUED => '',
      self::STATUS_PROCESSING => 'comment',
      self::STATUS_SUCCESS => 'info',
      self::STATUS_FAILURE => 'error',
      self::STATUS_FAILURE_RETRY => 'error',
    ];
    return isset($tags[$status]) ? $tags[$status] : NULL;
  }
