...
class SettingsForm extends ConfigFormBase {
  ...
  public function buildForm(array $form, FormStateInterface $form_state) {
    ...
    $form['queue_suspend_time'] = [
      '#type' => 'select',
      '#title' => $this->t('Queue suspend time'),
      '#description' => $this->t('The default queue suspend time when queue worker definition does not provide its own suspend time.'),
      '#options' => array_map([\Drupal::service('date.formatter'), 'formatInterval'], array_combine($options, $options)),
      '#default_value' => $config->get('queue.suspend_time'),
    ];
  ...
  public function submitForm(array &$form, FormStateInterface $form_state) {
    $this->config('advancedqueue.settings')
      ...
      ->set('queue.suspend_time', $form_state->getValue('queue_suspend_time'))
